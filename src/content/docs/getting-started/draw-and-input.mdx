...
- Press <Kbd other="Space" /> key to create pebbles. 
...
- Check if the player pressed the <Kbd other="Space" /> key, and call `create_pebble()`
...
If you run the game, and hit <Kbd other="Space" /> a bunch of times, it should look something like this:
...
Add a losing condition, like if you have 5 lives and each time you miss, you lose one. When you have 0, display a game over message. You'll probably want to want make <Kbd other="Space" /> key restart the game, which will require resetting the state and `pebbles.clear()` 
</Card>
